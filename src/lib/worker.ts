  ...
  if (data.type === "OPEN") {
    ...
    */
    ...
    const syncMessage = Backend.encodeSyncMessage(
      Backend.syncStart(backends[docId])
    );
    sendMessage({
      docId,
      source: workerId,
      encoded: { type: "sync", payload: syncMessage },
    });
  }
});
...
channel.addEventListener("message", (evt: any) => {
  ...
  const [newBackend, newSyncState, patch, outboundMessages] = receiveMessage(
    decoded,
    backend,
    syncState
  );
  ...
  outboundMessages.forEach((decoded) =>
    sendMessage({
      docId,
      source: workerId,
      target: source,
      encoded: encodeMessage(decoded),
    })
  );
});
